...
class IndexMinPQ(object):
	...
	def __init__(self, N):
		...
		self._keys = [-1] * (N+1)	# key = priority (ex: weight of an edge)
		self._pq = [-1] * (N+1)		# input: heap position 		output: index
		self._qp = [-1] * (N+1)		# input: index 				output: heap position 
...
class IndexMaxPQ(object):
	"max priority queue where each key is referenced by an index i"
	def __init__(self, N):
		"create indexed PQ with indices 0 thru N-1"
		self._NMAX = N              # max number of keys
		self._keys = [-1] * (N+1)	# key = priority (ex: weight of an edge)
	...
	def maxIndex(self):
		"return index associated with maximum key"
		return self._pq[1] - 1	# min index is stored in heap position 1; subtract 1 so indexing is from 0
	...
	def decreaseKey(self, i, key):
		...
		if (self.contains(i)): 				# 'contains' parameter is indexed from 0
			self._keys[i+1] = key
			self._sink(self._qp[i+1])		# qp[i] is the heap position; since key is less than original key, swim it up
		else:
			print "index is not in PQ"
	...
	def delMax(self):
		"delete the maximum key and return its associated index"
		assert self._N > 0
		if (self._N > 0):
			max_index = self._pq[1]
			self._exch(1, self._N)
			self._keys[max_index] = -1 		# remove key from keys
			self._pq[self._N] = -1 			# remove index from PQ
			self._qp[max_index] = -1 		# free heap position in heap
			self._N = self._N - 1			# reduce size of PQ by 1
			self._sink(1) 					# heapify heap
			return max_index - 1 			# subtract 1 so indexing is from 0
	...
	def _sink(self, k):
		...
		while (2*k <= self._N):								# check that node in heap position k has a child
			j = 2 * k 										# j is heap position of child
			if (j < self._N and self._less(j, j+1)): 	# set j to smallest child
				j = j + 1
			if (not self._less(k, j)): break				# compare parent to smallest child
			else:
				self._exch(k, j)
				k = j 										# set parent to heap position of child and repeat sink